...
#define RFM_UNKNOWN 0
#define RFM70       1
#define RFM73       2

extern volatile byte rfm7x_current_channel;

extern volatile byte rfm7x_mode;
#define RFM7x_MODE_TRANSMIT 1
#define RFM7x_MODE_RECEIVE  2

extern const unsigned char rfm7x_RX0_Address[5];
...
//! commands and registers of the RFM7x.
//!
...
//!
//! To use any of these functions, (except rfm7x_init() itself)
//! the interface and the module must have been initialized by
//! an rfm7x_init() call.
//!
//! Besides the registers shown here (bank 0) the rfm7x also has a
//! set of registers in bank 1. These bank 1 registers are initialized by
//! the rfm7x_init() call. If you want to do this yourself: the datasheet
...
//!
//! The high-level interface provides functions for using the rfm7x module.
...
//! version of this rfm7x library
//
//! \ingroup highlevel
//#define RFM7x_LIB_VERSION "V1.02 (2011-12-31)"

//! maximum number of data bytes in a (received or transmitted) rfm7x packet
//
//! \ingroup highlevel
#define RFM7x_MAX_PACKET_LEN  32

//! type of rfm7x (transmit or receive) buffer
//
//! \ingroup highlevel
typedef unsigned char rfm7x_buffer [ RFM7x_MAX_PACKET_LEN ];
...
//***************************************************************************//
//
// RFM7x register addresses
//
//***************************************************************************//

//! CONFIG : rfm7x configuration register
//
//! Bits (0 = LSB):
...
//! initialize the library and the rfm7x module
//
//! \ingroup lowlevel
//! This function must be called before any other rfm7x 
//! function is called. It can also be called later (maybe even
//! periodically) to re-initialize the interafce and the module.
//!
//! The rfm7x is initialized to
//! - 2 byte CRC
//! - power up
//! - receive mode
...
//added argument: module type (RFM70, RFM73)
void rfm7x_init( unsigned char );

//! read a single-byte command or register
//
//! \ingroup lowlevel
//! This function reads and returns the a single-byte (8 bit) 
//! RFM7x command or register reg. 
unsigned char rfm7x_register_read( unsigned char reg );

//! read a multi-byte command or register
//
//! \ingroup lowlevel
//! This function reads length bytes (8 bit each) from the RFM7x 
//! command or register reg into the buffer buf. 
void rfm7x_buffer_read(unsigned char reg, unsigned char *buf, unsigned char length);

//! write a single-byte command or register
//
//! \ingroup lowlevel
//! This function writes the single-byte (8 bit) val to
//! the  RFM7x command or register reg. 
void rfm7x_register_write( unsigned char reg, unsigned char val );

//! write a multi-byte command or register
//
//! \ingroup lowlevel
//! This function writes length bytes (8 bit each) from 
//! the buffer buf into the RFM7x 
//! command or register reg. 
void rfm7x_buffer_write( unsigned char reg, const unsigned char *buf, unsigned char length);
...
//! report whether the rfm7x module is present
//
//! \ingroup highlevel
//! This function tests whether the rfm7x module is present.
...
//! switch the rfm7x to transmit mode
//
//! \ingroup highlevel
//! This function flushes the transmit queue,
//! and switches the rfm7x to transmit mode.
void rfm7x_mode_transmit( void );

//! switch the rfm7x to receive mode
//
//! \ingroup highlevel
...
//! switch the rfm7x to standby mode
//
//! \ingroup highlevel
//! This function puts the rfm7x in standby I mode,
//! which reduces the power consumption 
//! (50 uA max).
//! rfm7x_mode_powerdown() reduces the power consumption
//! even further, but requires a longer (but unspecified?)
//! time to return to active mode.
void rfm7x_mode_standby( void );

//! switch the rfm7x to power down mode
//
//! \ingroup highlevel
//! This function puts the rfm7x in power down mode,
//! which reduces the power consumption to a minimum
//! ( 3 uA max). 
void rfm7x_mode_powerdown( void );

//! set the rfm7x lna gain to low
//
//! \ingroup highlevel
...
//! set the rfm7x lna gain to high
//
//! \ingroup highlevel
...
//! set the rfm7x channel frequency
//
//! \ingroup highlevel
//! This function sets the frequency (channel) used by the rfm7x for 
...
//! set the rfm7x air data rate (baudrate)
//
//! \ingroup highlevel
//! This function sets the air data rate used by the rfm7x for 
...
//! Note that this is the bitrate the rfm7x uses in the 
...
//! set the rfm7x CRC length
//
//! \ingroup highlevel
//! This function sets the length of the CRC used by the rfm7x in bytes.
...
//! set the rfm7x address length
//
//! \ingroup highlevel
//! This function sets the length (in bytes) of the addresses used by 
//! the rfm7x. Valid values are 3, 4 and 5.
...
//! set the rfm7x transmit power
//
//! \ingroup highlevel
//! This function sets the rfm7x transmitter output power level.
//! - level == 0 => -10 dBm
//! - level == 1 =>  -5 dBm
//! - level == 2 =>   0 dBm
//! - level == 3 =>  +5 dBm
...
//! is written (by calling rfm7x_channel() ) or 
//! explicitly by calling rfm7x_lost_packets_reset().
unsigned char rfm7x_lost_packets_count( void );

//! reset rfm7x lost packets count
//
//! \ingroup highlevel
//! This function resets the number of lost packets by reading and
//! re-writing the RF channel (RFM7x_REG_RF_CH register).
...
//! pipe must be in the range 0..5. 
//! A pipe > 5 has the same effect as using pipe 5. 
void rfm7x_pipe_enable( unsigned char d, unsigned char enabled );

//! set the rfm7x pipe 0 address
//
//! \ingroup highlevel
...
//! set the rfm7x pipe n (2..5) address
//
//! \ingroup highlevel
...
void rfm7x_channel_payload_size( unsigned char n, unsigned char size );

//! set the rfm7x transmit address
//
//! \ingroup highlevel
...
//! 
//! The RFM7x datasheet does not specify the value that is 
