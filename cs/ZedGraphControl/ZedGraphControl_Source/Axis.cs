...
*/
						...
						float topPix, double scaleFactor )
		{
			...
			double startVal = Math.Ceiling( (double) this.minScale / (double) this.step - 0.00000001 )
													* (double) this.step;
			...
			for ( int i=iStart; i<=iEnd; i++ )
			{
				if ( this.isLog )
					...
					dVal = startVal + (double) this.step * (double) i;
		...
		public void DrawMinorTics( Graphics g, int iStart, double scaleFactor, float topPix )
		{
			if ( this.isMinorTic && this.isVisible )
			{
				if ( this.isLog || this.minorStep < this.step )
				{
					...
					// go to the nearest even multiple of the step size
					// this makes sure that the minor tics are lined up with the major tics
					// in the event that the major tics don't start at exactly the beginning of the
					// axis (i.e., the min scale is 175, but the first major tic is at 200, stepping by 50 )
					dVal = Math.Ceiling( (double) this.minScale / (double) this.step - 0.00000001 )
						* (double) this.step - (double) this.step;
